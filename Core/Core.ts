...
export default class Core<T extends HTMLElement> extends Array<T> {
    private mutationObserver:MutationObserver;

    constructor( element:string | T | HTMLCollection ) {
    ...
    protected init( element:string | T | HTMLCollection ) {
        ...
        if ( element instanceof HTMLElement ) {
            ...
            for ( let i = 0; i < element.length; i++ )
                this.push( <T>element.item( i ) );

        } else {

            if ( element[ 0 ] == "<" && element[ ( <string>element ).length - 1 ] == ">" ) {
                ...
                this.push( <T>document.createElement( match[ 1 ] ) );

            } else {

                document.querySelectorAll( <string>element ).forEach( ( it:T ) => {
                    this.push( it );
                } );
            }
        }
    ...
    public static addClass( element:Element, className:string ) {

        if ( className != "" && className != null )
            className.split( " " ).forEach( it => element.classList.add( it ) );
    }

    public static removeClass( element:Element, className:string | Function ) {
    ...
    onRemove( callback:( number, HTMLElement ) => void ) {
    ...
    append( element:Element | Core<HTMLElement> ) {

        if ( element instanceof Core )
            element.forEach( it => this[ 0 ].appendChild( it ) );
        else
            this[ 0 ].appendChild( <Element>element );
    }
    ...
    on( name:string, callback:( event:Event ) => any ) {
    ...
    css( property:string | object, value?:string | number ):string | number | Core<T> {


        if ( value === undefined && Object.getPrototypeOf( property ) != Object ) {
            return Core.css( this[ 0 ], <string>property );
        }
        ...
        if ( Object.getPrototypeOf( property ) === Object ) {
            ...
            this.forEach( element => {
                Core.css( element, <string>property, value );
            } );
        }
    ...
    static css( element:HTMLElement, property:string, value?:string | number ):string | number {
    ...
    position() {
        ...
        return {
            top : offset.top - parentOffset.top - <number>this.css( "marginTop" ),
            left: offset.left - parentOffset.left - <number>this.css( "marginLeft" )
        };
    }
